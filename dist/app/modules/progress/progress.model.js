...
courseProgressSchema.pre('save', async function (next) {
    if (this.isModified('unlockedLectures') || this.isModified('completedLectures')) {
        try {
            ...
            // Calculate progress percentage based on unlocked lectures
            if (totalLectures > 0) {
                this.progressPercentage = Math.round((this.unlockedLectures.length / totalLectures) * 100);
            }
            else {
